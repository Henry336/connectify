      ...
      <aside className={`queue-pane ${sideTab === "chat" ? "chat-pane" : ""}`}>
        ...
        {sideTab === "chat" && <div className="room-chat">
          ...
          <form className="chat-compose" onSubmit={sendChat}>
            ...
            <textarea ref={chatInputRef} value={chatBody} onChange={(event) => setChatBody(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} maxLength={300} placeholder="Say something about this moment…" />
