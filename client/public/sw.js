const CACHE = "connectify-shell-v1";
self.addEventListener("install", (event) => { event.waitUntil(caches.open(CACHE).then((cache) => cache.addAll(["/", "/connectify.svg"]))); self.skipWaiting(); });
